...
export type DubinsCoordinates = [number, number, number];

export interface DubinsPath {
  /* the initial configuration */
  qi: DubinsCoordinates;
  /* the lengths of the three segments */
  segLength: DubinsCoordinates;
...
class Dubins {
  ...
  readonly _DEFAULT_CONFIG: DubinsCoordinates = [
    this._C_DEFAULT_DOUBLE,
    this._C_DEFAULT_DOUBLE,
    this._C_DEFAULT_DOUBLE,
  ];
  ...
  public getPath(start: DubinsCoordinates, end: DubinsCoordinates, turning_radius: number, step_size: number) {
    const path: number[][] = [];

    this.shortestAndSample(start, end, turning_radius, step_size, (q) => {
      path.push([q[0], q[1]]);
  ...
  private shortestAndSample(
    start: DubinsCoordinates,
    end: DubinsCoordinates,
    turning_radius: number,
    step_size: number,
    callback: (q: DubinsCoordinates, x: number) => ERROR_CODE,
  ) {
    const ret = this.dubins_shortest_path(start, end, turning_radius);

    return this.dubins_path_sample_many(ret[1], step_size, callback);
  }
  public dubins_shortest_path(
    user_FIRST: DubinsCoordinates,
    user_LAST: DubinsCoordinates,
    turning_radius: number,
  ): [ERROR_CODE, DubinsPath] {
    let errcode: ERROR_CODE;
    let segLength: DubinsCoordinates = this._DEFAULT_CONFIG;
    let cost: number;
  ...
  public dubins_intermediate_results(q0: DubinsCoordinates, q1: DubinsCoordinates, rho: number) {
  ...
  public dubins_path_sample_many(
    path: DubinsPath,
    stepSize: number,
    cb: (q: DubinsCoordinates, x: number) => ERROR_CODE,
  ): ERROR_CODE {
    let retcode: ERROR_CODE;
    let ret: [ERROR_CODE, DubinsCoordinates?];
    let q: DubinsCoordinates = this._DEFAULT_CONFIG;
    let x = 0.0;
    const length: number = this.dubins_path_length(path);
    while (x < length) {
      ret = this.dubins_path_sample(path, x, q);
      if (ret[0] === ERROR_CODE.EDUBOK) {
        q = ret[1] as DubinsCoordinates;
      }
      retcode = cb(q, x);
  ...
  public dubins_segment(
    t: number,
    qi: DubinsCoordinates,
    qt_i: DubinsCoordinates,
    type: SegmentType,
  ): DubinsCoordinates {
    if (qt_i.length < 3) throw new Error(`QT ${qt_i}`);
    const qt: DubinsCoordinates = qt_i.slice() as DubinsCoordinates;
  ...
  public dubins_path_sample(path: DubinsPath, t: number, q_i: DubinsCoordinates): [ERROR_CODE, DubinsCoordinates] {
    if (q_i.length < 3) throw new Error(`Q ${q_i}`);
    let q: DubinsCoordinates = q_i.slice() as DubinsCoordinates;
    /* tprime is the normalised variant of the parameter t */
    const tprime = t / path.rho;
    const qi: DubinsCoordinates = this._DEFAULT_CONFIG;
    let q1: DubinsCoordinates = this._DEFAULT_CONFIG; /* end-of segment 1 */
    let q2: DubinsCoordinates = this._DEFAULT_CONFIG; /* end-of segment 2 */
  ...
  public dubins_LSL(out_arg: DubinsCoordinates): [ERROR_CODE, DubinsCoordinates] {
    const out = out_arg.slice() as DubinsCoordinates;
    let tmp1: number;
  ...
  public dubins_RSR(out_arg: DubinsCoordinates): [ERROR_CODE, DubinsCoordinates] {
    const out = out_arg.slice() as DubinsCoordinates;
  ...
  public dubins_LSR(out_arg: DubinsCoordinates): [ERROR_CODE, DubinsCoordinates] {
    const out = out_arg.slice() as DubinsCoordinates;
    const p_sq: number =
  ...
  public dubins_RSL(out_arg: DubinsCoordinates): [ERROR_CODE, DubinsCoordinates] {
    const out = out_arg.slice() as DubinsCoordinates;
    const p_sq: number =
      -2 + this._NS_IN.d_sq + 2 * this._NS_IN.c_ab - 2 * this._NS_IN.d * (this._NS_IN.sa + this._NS_IN.sb);
    if (p_sq >= 0) {
  ...
  public dubins_RLR(out_arg: DubinsCoordinates): [ERROR_CODE, DubinsCoordinates] {
    const out = out_arg.slice() as DubinsCoordinates;
    const tmp0: number =
  ...
  public dubins_LRL(out_arg: DubinsCoordinates): [ERROR_CODE, DubinsCoordinates] {
    const out = out_arg.slice() as DubinsCoordinates;
    const tmp0: number =
  ...
  public dubins_word(pathType: DubinsPathType, out: DubinsCoordinates): [ERROR_CODE, DubinsCoordinates] {
    switch (pathType) {
